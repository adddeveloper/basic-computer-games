...
function drawPlayer() {
    ...
    if (player.x + player.v >= 0 && player.x + player.v <= window.innerWidth - player.w) {
        player.x += player.v;
    }
    // context.fillRect(player['x'], player['y'], player['w'], player['h']);
    // context.fill();
    context.drawImage(player.sprite, player['x']-10, player['y']-10, player['w']+20, player['h']+20);
...
class Enemy {
    ...
    draw() {
        ...
        context.fill();
        context.drawImage(this.sprite, this.x-10, this.y-10, this.width+20, this.height+20);
    }
    update(v) {
        this.y += v;
...
function drawscore(){
    ...
    context.fillText(JSON.stringify(score), 100*scale, 50*scale, 100*scale);
...
function drawButton(){
    context.drawImage(start,window.innerWidth/2-50*scale, 30*scale, 100*scale, 100*scale);
